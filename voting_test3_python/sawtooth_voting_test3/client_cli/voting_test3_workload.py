...
from sawtooth_voting_test3.client_cli.workload.workload_generator import \
    WorkloadGenerator
from sawtooth_voting_test3.client_cli.workload.sawtooth_workload import Workload
from sawtooth_voting_test3.client_cli.create_batch import create_voting_test3_transaction
from sawtooth_voting_test3.client_cli.create_batch import create_batch
from sawtooth_voting_test3.client_cli.exceptions import Voting_Test3CliException
from sawtooth_sdk.protobuf import batch_pb2

LOGGER = logging.getLogger(__name__)

Voting_Test3State = namedtuple('Voting_Test3State', ['name', 'url', 'value'])
...
class Voting_Test3Workload(Workload):
    """
    ...
    def __init__(self, delegate, args):
        super(Voting_Test3Workload, self).__init__(delegate, args)
        ...
        if args.key_file is not None:
            try:
                ...
                raise Voting_Test3CliException(str(pe))
            except IOError as ioe:
                raise Voting_Test3CliException(str(ioe))
        else:
            self._signer = crypto_factory.new_signer(
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        Voting_Test3State(name=name, url=url, value=0)
...
def do_workload(args):
    """
    Create WorkloadGenerator and Voting_Test3Workload. Set Voting_Test3 workload in
    generator and run.
    """
    try:
        args.auth_info = _get_auth_info(args.auth_user, args.auth_password)
        generator = WorkloadGenerator(args)
        workload = Voting_Test3Workload(generator, args)
