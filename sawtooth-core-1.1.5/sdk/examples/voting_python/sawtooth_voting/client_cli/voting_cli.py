...
from sawtooth_voting.client_cli.generate import do_generate
from sawtooth_voting.client_cli.populate import add_populate_parser
from sawtooth_voting.client_cli.populate import do_populate
from sawtooth_voting.client_cli.create_batch import add_create_batch_parser
from sawtooth_voting.client_cli.create_batch import do_create_batch
from sawtooth_voting.client_cli.load import add_load_parser
from sawtooth_voting.client_cli.load import do_load
from sawtooth_voting.client_cli.voting_workload import add_workload_parser
from sawtooth_voting.client_cli.voting_workload import do_workload

from sawtooth_voting.client_cli.voting_client import VotingClient
from sawtooth_voting.client_cli.exceptions import VotingCliException
from sawtooth_voting.client_cli.exceptions import VotingClientException
...
DISTRIBUTION_NAME = 'sawtooth-voting'
...
def add_set_parser(subparsers, parent_parser):
    message = 'Sends an voting transaction to set <name> to <value>.'

    parser = subparsers.add_parser(
        'set',
        parents=[parent_parser],
        description=message,
        help='Sets an voting value')
...
def add_inc_parser(subparsers, parent_parser):
    ...
    parser = subparsers.add_parser(
        ...
        parents=[parent_parser],
        description=message,
        help='Increments an voting value')
...
def add_dec_parser(subparsers, parent_parser):
    message = 'Sends an voting transaction to decrement <name> by <value>.'

    parser = subparsers.add_parser(
        'dec',
        parents=[parent_parser],
        description=message,
        help='Decrements an voting value')
...
def add_show_parser(subparsers, parent_parser):
    ...
    parser = subparsers.add_parser(
        ...
        parents=[parent_parser],
        description=message,
        help='Displays the specified voting value')
...
def add_list_parser(subparsers, parent_parser):
    message = 'Shows the values of all keys in voting state.'

    parser = subparsers.add_parser(
        'list',
        parents=[parent_parser],
        description=message,
        help='Displays all voting values')
...
def main(prog_name=os.path.basename(sys.argv[0]), args=None):
    ...
    if args.command == 'set':
        ...
        raise VotingCliException("invalid command: {}".format(args.command))
...
def main_wrapper():
    ...
    try:
        main()
    except (VotingCliException, VotingClientException) as err:
