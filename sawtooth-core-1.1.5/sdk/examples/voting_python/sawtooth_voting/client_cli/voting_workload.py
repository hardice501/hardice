...
from sawtooth_voting.client_cli.workload.workload_generator import \
    WorkloadGenerator
from sawtooth_voting.client_cli.workload.sawtooth_workload import Workload
from sawtooth_voting.client_cli.create_batch import create_voting_transaction
from sawtooth_voting.client_cli.create_batch import create_batch
from sawtooth_voting.client_cli.exceptions import VotingCliException
from sawtooth_sdk.protobuf import batch_pb2

LOGGER = logging.getLogger(__name__)

VotingState = namedtuple('VotingState', ['name', 'url', 'value'])
...
class VotingWorkload(Workload):
    ...
    def __init__(self, delegate, args):
        super(VotingWorkload, self).__init__(delegate, args)
        ...
        if args.key_file is not None:
            try:
                ...
                raise VotingCliException(str(pe))
            except IOError as ioe:
                raise VotingCliException(str(ioe))
        else:
            self._signer = crypto_factory.new_signer(
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        VotingState(name=name, url=url, value=0)
...
def do_workload(args):
    """
    Create WorkloadGenerator and VotingWorkload. Set Voting workload in
    generator and run.
    """
    try:
        args.auth_info = _get_auth_info(args.auth_user, args.auth_password)
        generator = WorkloadGenerator(args)
        workload = VotingWorkload(generator, args)
