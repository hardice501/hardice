...
VOTING_ADDRESS_PREFIX = hashlib.sha512(
    FAMILY_NAME.encode('utf-8')).hexdigest()[0:6]


def make_voting_address(name):
    return VOTING_ADDRESS_PREFIX + hashlib.sha512(
...
class VotingTransactionHandler(TransactionHandler):
    ...
    def apply(self, transaction, context):
        ...
        updated_state = _do_voting(verb, name, value, state)
...
def _do_voting(verb, name, value, state):
    verbs = {
        'set': _do_set,
        'inc': _do_inc,
        'dec': _do_dec,
